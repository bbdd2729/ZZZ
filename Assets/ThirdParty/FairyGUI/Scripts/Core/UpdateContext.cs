...
namespace FairyGUI
{
    ...
    public class UpdateContext
    {
        ...
        public void ApplyClippingProperties(Material mat, bool isStdMaterial)
        {
            ...
            if (stencilReferenceValue > 0)
            {
                mat.SetInt(ShaderConfig.ID_StencilComp, (int)CompareFunction.Equal);
                mat.SetInt(ShaderConfig.ID_Stencil, stencilCompareValue);
                mat.SetInt(ShaderConfig.ID_Stencil2, stencilCompareValue);
                mat.SetInt(ShaderConfig.ID_StencilOp, (int)StencilOp.Keep);
            ...
            {
                mat.SetInt(ShaderConfig.ID_StencilComp, (int)CompareFunction.Always);
        ...
        public void ApplyAlphaMaskProperties(Material mat, bool erasing)
        {
            if (!erasing)
            {
                if (stencilReferenceValue == 1)
                {
                    ...
                    mat.SetInt(ShaderConfig.ID_StencilOp, (int)StencilOp.Replace);
                    mat.SetInt(ShaderConfig.ID_StencilReadMask, 255);
                ...
                else
                {
                    if (stencilReferenceValue != 0 & _clipStack.Peek().reversed)
                        mat.SetInt(ShaderConfig.ID_StencilComp, (int)CompareFunction.NotEqual);
                    else
                        mat.SetInt(ShaderConfig.ID_StencilComp, (int)CompareFunction.Equal);
            ...
            else
            {
                if (stencilReferenceValue != 0 & _clipStack.Peek().reversed)
                {
                    ...
                    mat.SetInt(ShaderConfig.ID_StencilOp, (int)StencilOp.Zero);
