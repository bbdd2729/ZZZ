...
public class GameMain : MonoBehaviour
{
    ...
    public List<GameObject> playerModels = new(3); // 玩家模型列表
    ...
    {
        for (var i = 0; i < teamInfo.PlayerConfigList.Length; i++)
        {
            var playerConfig = teamInfo.PlayerConfigList[i];
            if (playerConfig == null || playerConfig.PlayerPrefab == null) continue;
            var go = Instantiate(playerConfig.PlayerPrefab, PlayerGenerateTargetPoint.position, Quaternion.identity);
