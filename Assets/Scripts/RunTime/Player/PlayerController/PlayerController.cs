...
{
    [Header("角色基本组件和属性")] [SerializeField] internal Animator               _animator;
    ...
    {
        get => playerControllerConfig.RotationSpeed;
    }

    public int AttackLength
    {
        get => playerConfig.AttackLength;
    }

    public float AnimationTranslationTime
    {
        get => playerControllerConfig.AnimationTranslationTime;
    }
    ...
    private void CreateStateMachineManually()
    {
        ...
        StateMachine.RegisterState(new RunEndState());
        StateMachine.RegisterState(new EvadeState());
        StateMachine.RegisterState(new EvadeEndState());
    ...
    public void SetCharacterRotation()
    {
        ...
        var targetAngle = Mathf.Atan2(input.x, input.y) * Mathf.Rad2Deg +
                          CameraSystem.Instance.CamRotation.eulerAngles.y;
